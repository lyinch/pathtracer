...
public:
    ...
    camera(Vec3 lookfrom = Vec3(0,0,0), Vec3 lookat = Vec3(0,0,-1), Vec3 vup = Vec3(0,1,0), float vfov = 90, float aspect = 2.0f, float aperture = 0, float focus_dist = 1, float t0 = 0.0f, float t1 = 0.0f){
        Vec3 u,v,w;
    ...
    ray get_ray(float u, float v){
            ...
            auto time = static_cast<float>(time0 + drand48() * (time1 - time0));
        return {origin+offset, top_left_corner + u * horizontal + v * vertical-origin-offset,time};
    };
